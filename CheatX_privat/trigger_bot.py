...
def trig():
    ...
    trig_key = open('%s\\CheatX\\cheatx-private\\settings\\t_k.cfg' % (appdata_p))
    read_key = trig_key.read()
    trigger_key = read_key

    file_work = '%s\\CheatX\\cheatx-private\\settings\\t\\tr.cfg' % (appdata_p)
    print(" Xtrigger has launched.")
    while True:
        ...
        if not GetWindowText(GetForegroundWindow()) == "Counter-Strike: Global Offensive - Direct3D 9":
            continue
